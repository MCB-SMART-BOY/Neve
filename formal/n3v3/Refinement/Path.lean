/-
  ...
  (crates/n3v3-std/src/io/mod.rs) to its Lean specification
  (Verify/Path.lean).
...
namespace n3v3.Refinement

open n3v3 (Path resolve_redirect_path safe_sentinel path_safe)
...
/--
  ...
  The Rust model has:
    path_rust_resolve_redirect_path cwd redirect =
      if List.any redirect (λ c => c = "..") then ["dev", "null", "n3v3-blocked-traversal"]
      else match cwd with | none => redirect | some cwd => cwd ++ redirect
  
  Since has_parent_dir redirect = List.any redirect (λ c => c = "..")
  and safe_sentinel = ["dev", "null", "n3v3-blocked-traversal"],
