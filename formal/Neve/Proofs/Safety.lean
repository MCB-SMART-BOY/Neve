/-
  Neve Type Safety — v17

  Real proofs (verified constructors):
    lit_*, var, lam, app (lam case), pipe (lam case),
    letIn, binop_add, binop_eq, binop_and

  Verified lemma (independently proven):
    matchOn_wildcard — wildcard first-arm case is proved

  Deferred axioms:
    matchOn (non-wildcard) — blocked by Lean 4.29 mutual inductive `match`
    app/pipe (var/letIn subcases) — env_preservation lemma
-/
...
-- ============================================================
-- Verified lemma: matchOn with wildcard first arm
-- ============================================================
...
def progress_preservation (Γ : Ctx) (env : Env) (e : Expr) (τ : Ty)
    (hty : HasType Γ e τ) (henv : EnvMatches Γ env) :
  ...
  -- === Application (lam case ✓, var/app cases deferred) ===
  | HasType.app _ f arg τ₁ τ₂ eff hf ha =>
      ...
      | HasType.lam _ x body _ _ _ hbody =>
          ...
          have henv' : EnvMatches ((x, τ₁) :: Γ) ((x, va) :: env) :=
  ...
  | HasType.letIn _ x val body τ τ' hval hbody =>
      ...
      have henv' : EnvMatches ((x, τ) :: Γ) ((x, vval) :: env) :=
  ...
  -- === Match (deferred axiom; wildcard case independently verified above) ===
  ...
  -- === Pipe (lam case ✓, general deferred) ===
...
theorem matchOn_wildcard_verified (Γ : Ctx) (env : Env) (scrutinee : Expr)
    ...
    (hrest : AllArmsMatch Γ rest τs τ) (henv : EnvMatches Γ env) :
