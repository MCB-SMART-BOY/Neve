...
for arg in "$@"; do
    case "$arg" in
        ...
        --all)
            cargo run -q -p n3v3 --example test_runner -- --clippy
            echo; cargo run -q -p n3v3 --example bug_hunt
            echo; cargo run -q -p n3v3 --example gen_diff_test -- -n 100 -d 4
            exit $? ;;
    esac
done

# Default: cargo test runner
cargo run -q -p n3v3 --example test_runner "$@"
