...
| REPL | ⚠️ Usable, not complete | Interactive flow now uses incremental HIR for the core path; project-local imports and `:load` file module graphs now work, but cross-root session switching and full tooling mirroring are still incomplete |
