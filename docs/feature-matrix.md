...
| 方法调用语法 `x.foo(y)` | ✅ | ✅ | ⚠️ | ✅ | ✅ | ⚠️ | AST/HIR runtime 都已可执行方法调用，AST 路径支持模块导入下的 method dispatch；类型系统与方法模型本身仍需继续稳定化 |
...
| `neve eval` | ⚠️ 可用 | 无 `import` 输入、以及常见 `std` item/module/glob 导入已默认走 frontend/HIR；非 `std` 导入仍会回退 AST |
