...
//
// Runs n3v3 expressions and verifies output matches expected values.
...
fn run_n3v3_eval(expr: &str) -> String {
    let tmp = std::env::temp_dir().join(format!("n3v3_diff_{}.n3v3", std::process::id()));
...
fn main() {
    ...
    for t in &tests {
        print!("  {} ... ", t.desc);
        let actual = run_n3v3_eval(t.expr);
