...
//
// Runs security regression tests by executing n3v3 scripts and
...
fn run_n3v3(source: &str) -> (String, String, i32) {
    let tmp = std::env::temp_dir().join(format!("n3v3_bug_hunt_{}.n3v3", std::process::id()));
    ...
    // Combine stdout and stderr for matching (n3v3 uses print/println)
...
fn check(test: &TestCase, bugs: &mut Vec<String>) {
    print!("  [{}] ... ", test.name);
    let (output, stderr, code) = run_n3v3(test.source);
