...
use n3v3_lsp::run_server;

/// Start the n3v3 Language Server.
/// 启动 n3v3 语言服务器。
...
pub fn version() -> Result<(), String> {
    println!("n3v3 LSP Version Info");
    println!("====================\n");
    println!("  Binary:    n3v3 {}", env!("CARGO_PKG_VERSION"));
    ...
    println!("\nEditor integration:");
    println!("  Helix:  n3v3 setup helix");
    println!("  VS Code: n3v3 setup vscode");
    println!("\nRun `n3v3 lsp --check` for a full health diagnostic.");
    Ok(())
}

pub fn check() -> Result<(), String> {
    println!("n3v3 LSP Health Check");
    println!("====================\n");

    // 1. Check that the LSP crate can be loaded.
    // 检查 LSP 包是否可以加载。
    println!("  ✓ n3v3-lsp crate loaded (v{})", env!("CARGO_PKG_VERSION"));

    // 2. Test document parsing.
    // 测试文档解析。
    let doc = n3v3_lsp::Document::new(
    ...
    // 3. Test diagnostics.
    // 测试诊断。
    let error_doc = n3v3_lsp::Document::new(
        "file:///error.n3v3".to_string(),
        "let x = 42;\nlet y = x + \"hello\";\n".to_string(),
    );
    ...
    for diag in &error_doc.diagnostics {
        println!(
            "    - {}: {}",
            match diag.severity {
                n3v3_lsp::DiagnosticSeverity::Error => "error",
                n3v3_lsp::DiagnosticSeverity::Warning => "warning",
                n3v3_lsp::DiagnosticSeverity::Note => "note",
            },
    ...
    // 测试语义 Token。
    let tokens =
        n3v3_lsp::generate_semantic_tokens_from_ast("let x = 42;\nfn add(a, b) = a + b;\n");
    ...
    // 5. Test formatting.
    // 测试格式化。
    let formatted = n3v3_fmt::format("let x=42;");
    ...
    // 6. Test symbol index.
    // 测试符号索引。
    let index_doc = n3v3_lsp::Document::new(
        "file:///index.n3v3".to_string(),
    ...
    if let Ok(home) = std::env::var("HOME") {
        let hd = std::path::PathBuf::from(&home)
            .join(".config")
            .join("helix");
        let grammar = hd.join("runtime").join("grammars").join("n3v3.so");
        let queries_dir = hd.join("runtime").join("queries").join("n3v3");
        ...
        if !has_grammar || !has_queries || !has_config {
            println!("\n  Run `n3v3 setup helix` to install Helix integration.");
        }
    }
