//! The `n3v3 explain` command — display extended error code documentation.
//! `n3v3 explain` 命令 — 显示扩展的错误代码文档。
//!
...
pub fn run(code: &str) -> Result<(), String> {
    explain(code).map_err(|e| {
        ...
        format!(
            "{}\n\nTip: use `n3v3 doc diagnostics` to browse all error codes.",
            e
        )
    })
}
