...
fn search_index(query: &str) -> Result<Vec<(String, String)>, String> {
    ...
    if let Ok(registry_url) = std::env::var("N3V3_REGISTRY") {
...
pub(crate) fn get_index_path() -> Option<PathBuf> {
    if let Ok(path) = std::env::var("N3V3_PACKAGE_INDEX") {
        return Some(PathBuf::from(path));
    }

    std::env::var("HOME")
        .ok()
        .map(|home| PathBuf::from(home).join(".n3v3").join("package-index.json"))
}
