//! The `n3v3 eval` command — canonical HIR pipeline.
//! `n3v3 eval` 命令 — 规范 HIR 管线。

use crate::output;
use n3v3_common::Span;
use n3v3_diagnostic::{Severity, emit};
use n3v3_eval::{Evaluator, Value, eval_error_to_diagnostic};
use n3v3_frontend::{LoadedSnippetModule, analyze_snippet_ast};
...
fn eval_value(
    ...
    let source_span = Span::new(
        n3v3_common::BytePos(0),
        n3v3_common::BytePos(source.len() as u32),
    );
...
fn emit_loaded_module_diagnostics(
    ...
    for diag in &entry.diagnostics {
        ...
        if diag.severity == Severity::Error && diag.kind.is_syntax() {
