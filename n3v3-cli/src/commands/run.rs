//! The `n3v3 run` command — canonical HIR pipeline.

use crate::{
    commands::{diagnostics, module_graph},
    output,
};
use n3v3_eval::{EvaluableModuleRef, Evaluator, Value, eval_error_to_diagnostic};
use n3v3_frontend::{FrontendDriver, ProgramAnalysis, ProgramParsedModule};
use n3v3_std::stdlib;
...
fn discover_flake_input_roots(start_dir: &Path) -> HashMap<String, PathBuf> {
    ...
    while current.parent().is_some() {
        ...
        if flake_path.exists() {
            match n3v3_config::flake::Flake::load(&current) {
                Ok(mut flake) => {
                ...
                Err(e) => {
                    output::warning(&format!(
                        "failed to load flake.n3v3 at {}: {e}",
                        current.display()
                    ));
                }
            }
...
/// Run an n3v3 source file through the canonical HIR pipeline and print the result.
/// 通过 canonical HIR 管线运行 n3v3 源文件并打印结果。
...
fn emit_program_parse_diagnostics(analysis: &ProgramAnalysis) -> Result<(), String> {
    ...
    diagnostics::emit_program_diagnostic_entries(&analysis.syntax_diagnostic_modules_in_order());
    if stats.parse_errors > 0 {
        // Message must start with "parse error" - `n3v3-cli/src/main.rs` maps that
        // prefix to exit code 2. A count-prefixed message fell through to exit 1.
        // 该消息必须以 "parse error" 开头：`n3v3-cli/src/main.rs` 依赖前缀映射退出码 2；
        // 之前以计数开头的消息会落到退出码 1。
        return Err(format!("parse error: {} diagnostic(s)", stats.parse_errors));
    }
    Ok(())
}
...
fn eval_modules_via_hir(analysis: &ProgramAnalysis) -> Result<Value, String> {
    ...
    evaluator
        ...
        .map_err(|e| {
            let diag = eval_error_to_diagnostic(&e, n3v3_common::Span::DUMMY);
            n3v3_diagnostic::emit("", "<runtime>", &diag);
            format!("eval error: {}", e)
        })
}
