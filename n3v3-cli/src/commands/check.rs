//! The `n3v3 check` command.
//! `n3v3 check` 命令。

use crate::{commands::module_graph, output};
use n3v3_diagnostic::{Severity, emit};
use n3v3_frontend::FrontendDriver;
use n3v3_hir::{ExprKind, ItemKind, StmtKind};
use n3v3_std::is_effectful_builtin;
...
fn discover_flake_input_roots(start_dir: &Path) -> HashMap<String, PathBuf> {
    ...
    while current.parent().is_some() {
        let flake_path = current.join("flake.n3v3");
        if flake_path.exists() {
            if let Ok(mut flake) = n3v3_config::flake::Flake::load(&current) {
...
/// Run type checking on an n3v3 file.
/// 对 n3v3 文件运行类型检查。
pub fn run(file: &str, verbose: bool, allow_effects: bool) -> Result<(), String> {
    ...
    for entry in analysis.diagnostic_modules_in_order() {
        for diag in &entry.diagnostics {
            ...
            match diag.severity {
                Severity::Error if diag.kind.is_syntax() => parse_errors += 1,
                Severity::Error => type_errors += 1,
        ...
        if verbose
            && !entry
                .diagnostics
                .iter()
                .any(|diag| diag.severity == Severity::Error && diag.kind.is_syntax())
            && let Some(ast) = analysis.parsed_source(entry.module_id)
        {
    ...
    if warnings > 0 {
        output::success(&format!("OK - No errors found ({warnings} warning(s))"));
    } else {
...
struct EffectContext<'a> {
    method_resolutions: &'a HashMap<n3v3_common::Span, n3v3_hir::DefId>,
    effectful_definitions: &'a HashSet<n3v3_hir::DefId>,
}

fn collect_effectful_calls(
    module: &n3v3_hir::Module,
    method_resolutions: &HashMap<n3v3_common::Span, n3v3_hir::DefId>,
    effectful_definitions: &HashSet<n3v3_hir::DefId>,
    out: &mut Vec<(String, n3v3_common::Span)>,
) {
...
fn walk_expr(
    ...
    match &expr.kind {
        ...
        ExprKind::Interpolated(parts) => {
            for part in parts {
                if let n3v3_hir::StringPart::Expr(e) = part {
                    walk_expr(e, context, out);
                }
            }
        }
...
mod tests {
    ...
    fn run_check(source: &str, allow_effects: bool) -> Result<(), String> {
        ...
        let path = dir.path().join("main.n3v3");
...
"#,
    ...
    fn check_rejects_imported_effectful_function() {
        let dir = tempdir().expect("temporary directory should be created");
        let dependency = dir.path().join("dependency.n3v3");
        let main = dir.path().join("main.n3v3");
        std::fs::write(
            &dependency,
