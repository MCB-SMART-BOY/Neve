...
use crate::output;
use n3v3_config::flake::{Flake, FlakeLock};
use n3v3_derive::Hash;
use n3v3_fetch::{git as fetch_git, url as fetch_url, verify as fetch_verify};
use std::fs;
...
pub fn run() -> Result<(), String> {
    ...
    // 在当前目录中查找 flake
    let flake_path = Path::new("flake.n3v3");

    if !flake_path.exists() {
        return Err("no flake.n3v3 found in current directory".to_string());
    }
...
fn update_input(
    ...
    branch: Option<&str>,
    tag: Option<&str>,
) -> Result<n3v3_config::flake::FlakeLockEntry, String> {
    ...
    Ok(n3v3_config::flake::FlakeLockEntry {
        name,
        url: resolved_url,
        hash,
        last_modified,
...
fn resolve_git_source(url: &str, git_ref: &str) -> Result<(String, Hash), String> {
    let work_dir = temp_work_dir("n3v3-update-git")?;
