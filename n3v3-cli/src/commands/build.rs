//! The `n3v3 build` command.
//! `n3v3 build` 命令。
//!
//! Builds a package from an n3v3 file or flake.
//! 从 n3v3 文件或 flake 构建软件包。

use crate::commands::{diagnostics, module_graph};
use crate::output;
use crate::platform::{BuildBackend, PlatformCapabilities, warn_limited_sandbox};
use n3v3_builder::{BuildBackend as BuilderBackend, Builder, BuilderConfig};
use n3v3_config::flake::{Flake, FlakeOutput};
use n3v3_derive::{Derivation, StorePath};
use n3v3_eval::{EvaluableModuleRef, Evaluator, Value};
use n3v3_frontend::{FrontendDriver, ProgramAnalysis};
use n3v3_std::stdlib;
use n3v3_store::{BinaryCache, CacheConfig, Store};
    ...
    let (source_path, target_attr) = match package {
        Some(pkg) => {
            ...
            if pkg.contains('.') || pkg.starts_with('/') || pkg.starts_with("./") {
                ...
                // 假设它是当前 flake/文件中的属性
                ("flake.n3v3".to_string(), Some(pkg.to_string()))
            }
        }
        None => {
            // Look for default file in current directory
            // 在当前目录中查找默认文件
            if Path::new("flake.n3v3").exists() {
                ("flake.n3v3".to_string(), None)
            } else if Path::new("default.n3v3").exists() {
                ("default.n3v3".to_string(), None)
            } else {
                return Err("no flake.n3v3 or default.n3v3 found in current directory".to_string());
            }
        }
    };
...
fn resolve_derivations_for_source(
    ...
    if path.file_name().is_some_and(|name| name == "flake.n3v3") {
...
impl BuildCacheSettings {
    fn from_inputs(
        ...
        if cache_urls.is_empty() {
            cache_urls = parse_env_list("N3V3_BINARY_CACHE_URLS");
        }
        ...
        if cache_dirs.is_empty() {
            cache_dirs = parse_env_list("N3V3_BINARY_CACHE_LOCAL_DIRS")
                .into_iter()
                .map(PathBuf::from)
                .collect();
        }
        ...
        Self {
            cache_urls,
            cache_dirs,
            substitute_enabled: cli_substitute && parse_env_bool("N3V3_SUBSTITUTE", true),
            upload_enabled: cli_cache_upload || parse_env_bool("N3V3_BINARY_CACHE_UPLOAD", false),
            public_keys: if cli_cache_public_keys.is_empty() {
                parse_env_keys(
                    "N3V3_BINARY_CACHE_PUBLIC_KEYS",
                    "N3V3_BINARY_CACHE_PUBLIC_KEY",
                )
            } else {
                cli_cache_public_keys
                    .iter()
            ...
            private_keys: if cli_cache_private_keys.is_empty() {
                parse_env_keys(
                    "N3V3_BINARY_CACHE_PRIVATE_KEYS",
                    "N3V3_BINARY_CACHE_PRIVATE_KEY",
                )
            } else {
                cli_cache_private_keys
                    .iter()
...
mod tests {
    ...
    fn test_candidate_output_paths_for_fixed_output() {
        ...
        let drv = Derivation::builder("demo", "1.0.0")
            .output(Output::fixed("out", hash, n3v3_derive::HashMode::Recursive))
            .build();
...
"#,
        ...
        let derivations = resolve_derivations_for_source(&root.join("default.n3v3"), None).unwrap();
...
"#
        ...
        let derivations = resolve_derivations_for_source(&root.join("flake.n3v3"), None).unwrap();
        assert_eq!(derivations.len(), 1);
...
"#
        ...
        let derivations =
            resolve_derivations_for_source(&root.join("flake.n3v3"), Some("demo")).unwrap();
        assert_eq!(derivations.len(), 1);
        assert_eq!(derivations[0].name, "demo");
        assert_eq!(derivations[0].version, "3.1.0");
    }
