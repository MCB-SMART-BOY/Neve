...
/// `error: could not compile `file.n3v3` due to 3 previous errors`
...
pub(super) fn count_diagnostics(diagnostics: &[Diagnostic]) -> (usize, usize) {
    ...
    for diag in diagnostics {
        match diag.severity {
            n3v3_diagnostic::Severity::Error => errors += 1,
            n3v3_diagnostic::Severity::Warning => warnings += 1,
            _ => {}
        }
    }
    (errors, warnings)
