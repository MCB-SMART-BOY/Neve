    ...
    let (token_type, modifiers) = match &token.kind {
        ...
        TokenKind::Ident(name) => {
            if ctx.after_fn {
                ...
                // Variable definition (readonly in n3v3 since it's immutable)
                // 变量定义（在 n3v3 中是只读的，因为它是不可变的）
                (
...
fn classify_ast_pattern(
    ...
    pattern: &n3v3_syntax::Pattern,
    token_type: u32,
    modifiers: u32,
) {
    use n3v3_syntax::PatternKind;
