...
use n3v3_syntax::{self as ast, PatternKind as AstPatternKind};
...
fn find_hir_item_for_ast<'a>(ast_item: &ast::Item, hir: &'a Module) -> Option<&'a n3v3_hir::Item> {
    hir.items
        .iter()
        .find(|hir_item| hir_item_matches_ast(ast_item, hir_item))
}

fn hir_item_matches_ast(ast_item: &ast::Item, hir_item: &n3v3_hir::Item) -> bool {
...
fn callable_type_string(
    module: &Module,
    generics: &[n3v3_hir::GenericParam],
    params: &[HirParam],
    ret: &Ty,
) -> String {
    let mut ty = Ty {
    ...
    match (&ast_expr.kind, &hir_expr.kind) {
        ...
        (ast::ExprKind::Interpolated(parts), HirExprKind::Interpolated(hir_parts)) => {
            for (ast_part, hir_part) in parts.iter().zip(hir_parts) {
                if let (ast::StringPart::Expr(ast_expr), n3v3_hir::StringPart::Expr(hir_expr)) =
                    (ast_part, hir_part)
                {
                    collect_expr_hovers(
                        ast_expr,
