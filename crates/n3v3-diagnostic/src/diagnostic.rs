...
impl DiagnosticKind {
    /// Whether this diagnostic is produced before type checking - by the lexer or
    /// the parser. `n3v3 check`/`n3v3 run` report these as syntax ("parse") errors
    /// and exit 2, so both kinds must be classified together; treating only
    /// `Parser` as syntax reported lexer errors as type errors (exit 3).
    /// 该诊断是否产生于类型检查之前（词法或语法阶段）。`n3v3 check`/`n3v3 run` 把它们
    /// 作为语法（“parse”）错误报告并退出 2，因此两类必须一起归类；此前的实现只认
    /// `Parser`，导致词法错误被报成类型错误（退出 3）。
    pub fn is_syntax(&self) -> bool {
        matches!(self, Self::Lexer | Self::Parser)
    }
}
