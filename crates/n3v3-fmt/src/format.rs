...
impl Formatter {
    ...
    fn format_let(&self, p: &mut Printer, def: &LetDef) {
        ...
        // The top-level `let` is optional only for a plain variable or wildcard
        // pattern (`x = 1`, `_ = 1`). A tuple/list pattern needs the keyword:
        // `(x, y) = t` is a parse error, so omitting `let` printed a file that no
        // longer parses.
        // v4.0：顶层 `let` 只对普通变量或通配符模式可省略（`x = 1`、`_ = 1`）。
        // 元组/列表模式必须保留关键字：`(x, y) = t` 是解析错误，省略 `let` 会打印出
        // 无法再解析的文件。
        if !matches!(
            def.pattern.kind,
    ...
    fn format_expr(&self, p: &mut Printer, expr: &Expr) {
        match &expr.kind {
            ...
            ExprKind::Lambda {
                ...
                // `||` lexes as the logical-or operator, so an empty parameter list
                // has no `|...| ` spelling - emitting `||` produced output that does
                // not parse. Use the `fn()` form instead.
                // `||` 被词法分析为逻辑或运算符，空参数列表没有 `|...|` 写法；此前打印
                // `||` 会生成无法解析的输出，改用 `fn()` 形式。
                if params.is_empty() {
                    p.write("fn()");
                } else {
                ...
                p.write(" ");
                // After lambda parameters a `{` starts a *block*, not a record (see
                // parser `parse_lambda_body`), so a record body must be parenthesized
                // or the printed output parses as a block and then fails on the
                // field commas. `RecordUpdate` prints as `{ base | fields }` too.
                // lambda 参数之后紧跟的 `{` 会被解析为块而不是记录（见 parser 的
                // `parse_lambda_body`），因此记录体必须加括号，否则打印结果按块解析，
                // 遇到字段逗号即报错。`RecordUpdate` 同样打印为 `{ base | fields }`。
                let body_needs_parens = matches!(
                    body.kind,
                    ExprKind::Record(_) | ExprKind::RecordUpdate { .. }
                );
    ...
    fn format_pattern(&self, p: &mut Printer, pattern: &Pattern) {
        match &pattern.kind {
            ...
            PatternKind::Tuple(patterns) => {
                ...
                // `(x,)` destructures a 1-tuple; without the trailing comma the
                // pattern prints as `(x)` and binds the whole tuple instead - same
                // rule as the expression printer above.
                // `(x,)` 匹配一元组；缺少尾逗号时会打印成 `(x)`，绑定含义变成整个元组
                // ——与上面的表达式打印器同一条规则。
                if patterns.len() == 1 {
                    p.write(",");
                }
            ...
            PatternKind::ListRest { init, rest, tail } => {
                ...
                // The `..` always follows the init patterns, whether or not it is
                // named (`[x, ..]`, `[x, ..rest]`) - so the separator depends on
                // `init` alone. Gating it on rest/tail printed `[x..]`, which does
                // not parse.
                // `..` 始终跟在 init 之后（`[x, ..]`、`[x, ..rest]`），分隔符只取决
                // 于 init 是否为空；先前按 rest/tail 判定会打印出无法解析的 `[x..]`。
                if !init.is_empty() {
                    p.write(", ");
                }
                if let Some(r) = rest {
