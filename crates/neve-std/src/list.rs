...
pub fn builtins() -> Vec<(&'static str, Value)> {
    vec![
        // Basic operations
        ("list.empty", Value::Builtin(BuiltinFn {
            name: "list.empty",
            arity: 0,
            func: |_args| Ok(Value::List(Rc::new(Vec::new()))),
        })),
        ("list.singleton", Value::Builtin(BuiltinFn {
            name: "list.singleton",
            arity: 1,
            func: |args| Ok(Value::List(Rc::new(vec![args[0].clone()]))),
        })),
        ("list.len", Value::Builtin(BuiltinFn {
            name: "list.len",
            arity: 1,
            func: |args| {
                match &args[0] {
        ...
        ("list.isEmpty", Value::Builtin(BuiltinFn {
            name: "list.isEmpty",
            arity: 1,
            func: |args| {
                match &args[0] {
        ...
        // Access operations
        ("list.head", Value::Builtin(BuiltinFn {
            name: "list.head",
            arity: 1,
            func: |args| {
                match &args[0] {
                    Value::List(items) => {
                        Ok(items.first().cloned().map(|v| Value::Some(Box::new(v))).unwrap_or(Value::None))
                    }
                    _ => Err("list.head expects a list".to_string()),
                }
            },
        })),
        ("list.tail", Value::Builtin(BuiltinFn {
            name: "list.tail",
            arity: 1,
            func: |args| {
                match &args[0] {
        ...
        ("list.last", Value::Builtin(BuiltinFn {
            name: "list.last",
            arity: 1,
            func: |args| {
                match &args[0] {
                    Value::List(items) => {
                        Ok(items.last().cloned().map(|v| Value::Some(Box::new(v))).unwrap_or(Value::None))
                    }
                    _ => Err("list.last expects a list".to_string()),
                }
            },
        })),
        ("list.init", Value::Builtin(BuiltinFn {
            name: "list.init",
            arity: 1,
            func: |args| {
                match &args[0] {
        ...
        ("list.get", Value::Builtin(BuiltinFn {
            name: "list.get",
            arity: 2,
            func: |args| {
                match (&args[0], &args[1]) {
                    (Value::Int(idx), Value::List(items)) => {
                        ...
                        Ok(items.get(idx).cloned().map(|v| Value::Some(Box::new(v))).unwrap_or(Value::None))
                    }
        ...
        ("list.cons", Value::Builtin(BuiltinFn {
            name: "list.cons",
            arity: 2,
            func: |args| {
                match &args[1] {
        ...
        ("list.append", Value::Builtin(BuiltinFn {
            name: "list.append",
            arity: 2,
            func: |args| {
                match (&args[0], &args[1]) {
        ...
        ("list.reverse", Value::Builtin(BuiltinFn {
            name: "list.reverse",
            arity: 1,
            func: |args| {
                match &args[0] {
        ...
        ("list.take", Value::Builtin(BuiltinFn {
            name: "list.take",
            arity: 2,
            func: |args| {
                match (&args[0], &args[1]) {
        ...
        ("list.drop", Value::Builtin(BuiltinFn {
            name: "list.drop",
            arity: 2,
            func: |args| {
                match (&args[0], &args[1]) {
        ...
        ("list.map", Value::Builtin(BuiltinFn {
            name: "list.map",
            arity: 2,
            func: |_args| {
        ...
        ("list.filter", Value::Builtin(BuiltinFn {
            name: "list.filter",
            arity: 2,
            func: |_args| {
                Err("list.filter requires runtime closure evaluation".to_string())
            },
        })),
        ("list.fold", Value::Builtin(BuiltinFn {
            name: "list.fold",
            arity: 3,
            func: |_args| {
                Err("list.fold requires runtime closure evaluation".to_string())
            },
        })),
        ("list.foldRight", Value::Builtin(BuiltinFn {
            name: "list.foldRight",
            arity: 3,
            func: |_args| {
                Err("list.foldRight requires runtime closure evaluation".to_string())
            },
        })),

        // Aggregation
        ("list.sum", Value::Builtin(BuiltinFn {
            name: "list.sum",
            arity: 1,
            func: |args| {
                match &args[0] {
        ...
        ("list.product", Value::Builtin(BuiltinFn {
            name: "list.product",
            arity: 1,
            func: |args| {
                match &args[0] {
                    Value::List(items) => {
                        ...
                        for item in items.iter() {
                            match item {
                                ...
                                _ => return Err("list.product expects a list of integers".to_string()),
                            }
                        }
        ...
        ("list.max", Value::Builtin(BuiltinFn {
            name: "list.max",
            arity: 1,
            func: |args| {
                match &args[0] {
                    Value::List(items) => {
                        ...
                        Ok(max.map(|m| Value::Some(Box::new(Value::Int(m)))).unwrap_or(Value::None))
                    }
                    _ => Err("list.max expects a list".to_string()),
                }
            },
        })),
        ("list.min", Value::Builtin(BuiltinFn {
            name: "list.min",
            arity: 1,
            func: |args| {
                match &args[0] {
                    Value::List(items) => {
                        ...
                        Ok(min.map(|m| Value::Some(Box::new(Value::Int(m)))).unwrap_or(Value::None))
                    }
        ...
        // Search
        ("list.contains", Value::Builtin(BuiltinFn {
            name: "list.contains",
            arity: 2,
            func: |args| {
                match &args[1] {
        ...
        ("list.indexOf", Value::Builtin(BuiltinFn {
            name: "list.indexOf",
            arity: 2,
            func: |args| {
                match &args[1] {
        ...
        // Sorting
        ("list.sort", Value::Builtin(BuiltinFn {
            name: "list.sort",
            arity: 1,
            func: |args| {
                match &args[0] {
        ...
        // Conversion
        ("list.range", Value::Builtin(BuiltinFn {
            name: "list.range",
            arity: 2,
            func: |args| {
                match (&args[0], &args[1]) {
        ...
        ("list.replicate", Value::Builtin(BuiltinFn {
            name: "list.replicate",
            arity: 2,
            func: |args| {
                match &args[0] {
        ...
        // Zipping
        ("list.zip", Value::Builtin(BuiltinFn {
            name: "list.zip",
            arity: 2,
            func: |args| {
                match (&args[0], &args[1]) {
        ...
        ("list.unzip", Value::Builtin(BuiltinFn {
            name: "list.unzip",
            arity: 1,
            func: |args| {
                match &args[0] {
