...
impl SandboxConfig {
    ...
    pub fn new(root: PathBuf) -> Self {
        ...
        Self {
            store_dir: PathBuf::from("/n3v3/store"),
...
impl Sandbox {
    ...
    fn execute_with_namespaces(
        ...
        let stdout_log = self.config.build_dir.join(".n3v3-sandbox-stdout");
        let stderr_log = self.config.build_dir.join(".n3v3-sandbox-stderr");
        ...
        match unsafe { fork() } {
            ...
            Ok(ForkResult::Child) => {
                ...
                match output {
                    Ok(o) => {
                        let _ = std::fs::write("/build/.n3v3-sandbox-stdout", &o.stdout);
                        let _ = std::fs::write("/build/.n3v3-sandbox-stderr", &o.stderr);
                        std::process::exit(o.status.code().unwrap_or(1));
                    }
                    Err(e) => {
                        let msg = format!("Failed to execute {}: {}", program, e);
                        let _ = std::fs::write("/build/.n3v3-sandbox-stderr", msg.as_bytes());
                        std::process::exit(1);
                    }
                }
            }
