...
/// 构建使用的默认 Docker 镜像。
pub const DEFAULT_BUILD_IMAGE: &str = "n3v3-build:latest";
...
RUN mkdir -p /n3v3/store /build /output /tmp

# Set up environment
ENV PATH="/n3v3/store/bin:/usr/local/bin:/usr/bin:/bin"
ENV HOME="/build"
ENV TMPDIR="/tmp"
...
impl DockerExecutor {
    ...
    pub fn ensure_image(&self) -> Result<(), BuildError> {
        ...
        if self.config.auto_build {
            ...
            return Err(BuildError::Sandbox(format!(
                "Docker image '{}' not found. Run 'n3v3 docker build-image' to create it.",
                self.config.image
            )));
        }
    ...
    pub fn execute(
        ...
        // 准备卷
        let mut volumes = vec![
            format!("{}:/n3v3/store:ro", self.store_dir.display()),
