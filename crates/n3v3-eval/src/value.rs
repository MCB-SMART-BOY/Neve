...
//! 运行时值。
//!
//! This module defines all value types that can exist during n3v3 program execution.
//! 本模块定义了 n3v3 程序执行过程中可能存在的所有值类型。
//!
...
use crate::EvalError;
use n3v3_common::{Int, int_to_f64};
use n3v3_hir::{Expr, Param};
...
impl Value {
    ...
    /// 检查值是否为真值。
    ///
    /// In n3v3, only `false` and `None` are falsy.
    /// 在 n3v3 中，只有 `false` 和 `None` 是假值。
...
impl KeyCtx {
    ...
    fn ty_key(&mut self, ty: &n3v3_hir::Ty) -> String {
        use n3v3_hir::TyKind;
        match &ty.kind {
...
mod tests {
    ...
    fn redirected_command_runtime_object_tracks_embedded_redirects_in_key() {
        let command = Value::command_object_with_options_and_redirects(
            ...
            HashMap::new(),
            vec![
                super::RedirectValue::stdout_path("/tmp/n3v3.out"),
                super::RedirectValue::stderr_path("/tmp/n3v3.err"),
            ],
        );

        assert_eq!(
            stable_key(&command),
            "Command{program=\"echo\",args=[\"hello\"],cwd=None,stdin=None,env={},redirects=[Redirect{stream=stdout,path=\"/tmp/n3v3.out\"},Redirect{stream=stderr,path=\"/tmp/n3v3.err\"}]}"
        );
    }
    ...
    fn redirected_pipeline_runtime_object_tracks_embedded_redirects_in_key() {
        ...
        assert_eq!(
            stable_key(&pipeline),
            "Pipeline{commands=[Command{program=\"printf\",args=[\"hello\"],cwd=None,stdin=None,env={}},Command{program=\"cat\",args=[],cwd=None,stdin=None,env={}}],redirects=[Redirect{stream=stdout,path=\"/tmp/n3v3.out\"},Redirect{stream=stderr,path=\"/tmp/n3v3.err\"}]}"
        );
    }

    #[test]
    fn redirect_runtime_object_has_stable_non_path_key() {
        let redirect = Value::redirect_stdout_path_object("/tmp/n3v3.out");
        let redirect_key = stable_key(&redirect);
        let path_key = stable_key(&Value::Path(Rc::new(PathBuf::from("/tmp/n3v3.out"))));

        assert_eq!(
            redirect_key,
            "Redirect{stream=stdout,path=\"/tmp/n3v3.out\"}"
        );
    ...
    fn redirect_stderr_runtime_object_has_distinct_stable_key() {
        let stdout_redirect = Value::redirect_stdout_path_object("/tmp/n3v3.out");
        let stderr_redirect = Value::redirect_stderr_path_object("/tmp/n3v3.out");

        assert_eq!(
            stable_key(&stderr_redirect),
            "Redirect{stream=stderr,path=\"/tmp/n3v3.out\"}"
        );
    ...
    fn redirect_stdin_runtime_object_has_distinct_stable_key() {
        let stdout_redirect = Value::redirect_stdout_path_object("/tmp/n3v3.in");
        let stdin_redirect = Value::redirect_stdin_path_object("/tmp/n3v3.in");

        assert_eq!(
            stable_key(&stdin_redirect),
            "Redirect{stream=stdin,path=\"/tmp/n3v3.in\"}"
        );
