...
use n3v3_common::{Span, int_is_negative, int_is_zero, int_to_f64, int_to_u32, int_to_usize};
...
impl Evaluator {
    ...
    pub fn eval(&mut self, expr: &Expr) -> Result<Value, EvalError> {
        match &expr.kind {
            ...
            ExprKind::Interpolated(parts) => {
                ...
                for part in parts {
                    match part {
                        n3v3_hir::StringPart::Literal(s) => result.push_str(s),
                        n3v3_hir::StringPart::Expr(e) => {
                            let val = self.eval(e)?;
    ...
    fn apply_evaluator_owned_builtin(
        ...
        match name {
            ...
            "read" | "write" | "cmd" | "env" | "exec" | "run" | "sh" => {
                Ok(None) // handled by n3v3-std extra_builtins
            }
            ...
            "io.tempDir" => {
                ...
                Ok(Some(self.builtin_temp_dir(&args[0])?))
            }
            "io.readFileLines" => {
                if args.len() != 2 {
                    return Err(EvalError::WrongArity);
    ...
    /// Evaluate a callback inside a temporary directory and remove it afterwards.
    /// 在临时目录中求值回调，并在之后删除该目录。
    fn builtin_temp_dir(&mut self, callback: &Value) -> Result<Value, EvalError> {
        let directory =
            tempfile::tempdir().map_err(|e| EvalError::TypeError(format!("io.tempDir: {e}")))?;
        let directory_path = directory.keep();
        let path_value = Value::Path(Rc::new(directory_path.clone()));
        let result = self.apply(callback.clone(), vec![path_value]);
        let cleanup = std::fs::remove_dir_all(&directory_path).or_else(|error| {
        ...
        match (result, cleanup) {
            ...
            (Ok(_), Err(error)) => {
                Err(EvalError::TypeError(format!("io.tempDir cleanup: {error}")))
            }
            (Err(error), Ok(())) => Err(error),
            (Err(callback_error), Err(cleanup_error)) => Err(EvalError::TypeError(format!(
                "io.tempDir callback failed: {callback_error}; cleanup failed: {cleanup_error}"
            ))),
        }
    }
    ...
    fn eval_stmt(&mut self, stmt: &n3v3_hir::Stmt) -> Result<(), EvalError> {
        match &stmt.kind {
            n3v3_hir::StmtKind::Let { pattern, value, .. } => {
                let val = self.eval(value)?;
                let bindings = self
                    .match_pattern(pattern, &val)
                    .ok_or(EvalError::PatternMatchFailed)?;
    ...
    fn match_pattern(
        &self,
        pattern: &n3v3_hir::Pattern,
        value: &Value,
    ) -> Option<Vec<(LocalId, Value)>> {
        use n3v3_hir::PatternKind;

        // Pre-calculate expected binding count to reduce allocations
        // 预先计算预期绑定数量以减少分配
        fn estimate_bindings(pattern: &n3v3_hir::Pattern) -> usize {
            use n3v3_hir::PatternKind;
            match &pattern.kind {
