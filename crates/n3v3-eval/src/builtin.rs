...
//! 内置函数。
//!
//! This module provides all built-in functions available in n3v3.
//! 本模块提供 n3v3 中所有可用的内置函数。
//!
//! ## Categories 分类
//!
    ...
    vec![
        ...
        (
            ...
            Value::Builtin(BuiltinFn {
                name: "toString",
                arity: 1,
                func: |args| Ok(Value::String(Rc::new(format_value(&args[0])))),
            }),
        ),
        (
            "toInt",
        ...
        (
            ...
            Value::Builtin(BuiltinFn {
                ...
                func: |args| match &args[0] {
                    Value::List(items) => {
                        let words: Result<Vec<_>, _> = items
                            .iter()
                            .map(|v| {
        ...
        (
            ...
            Value::Builtin(BuiltinFn {
                ...
                func: |args| match &args[0] {
                    Value::List(items) => {
                        let lines: Result<Vec<_>, _> = items
                            .iter()
                            .map(|v| {
                                if let Value::String(s) = v {
                                    Ok(s.as_str())
        ...
        (
            ...
            Value::Builtin(BuiltinFn {
                ...
                func: |args| match (&args[0], &args[1], &args[2]) {
                    (Value::String(from), Value::String(to), Value::String(s)) => Ok(
        ...
        (
            ...
            Value::Builtin(BuiltinFn {
                ...
                arity: 2,
                func: |args| match (&args[0], &args[1]) {
                    (Value::String(key), Value::Record(fields)) => match fields.get(key.as_str()) {
                    ...
                    match &args[0] {
                        Value::Thunk(thunk) => {
                            ...
                            match &*thunk.state() {
                                ...
                                ThunkState::Failed(e) => Err(e.to_string()),
                                ThunkState::HirUnevaluated { .. } | ThunkState::Evaluating => {
                    ...
                    match &args[0] {
                        Value::Record(attrs) => {
                            ...
                            let name = match attrs.get("name") {
                                ...
                                Some(_) => {
                                    return Err("derivation 'name' must be a string".to_string());
                                }
                            ...
                            let builder = match attrs.get("builder") {
                                Some(Value::String(s)) => s.to_string(),
                                Some(_) => {
                                    return Err("derivation 'builder' must be a string".to_string());
                                }
                                None => {
                                    return Err("derivation requires 'builder' field".to_string());
                                }
                            };
                            ...
                            let system = match attrs.get("system") {
                                Some(Value::String(s)) => s.to_string(),
                                Some(_) => {
                                    return Err("derivation 'system' must be a string".to_string());
                                }
                                None => {
                                    return Err("derivation requires 'system' field".to_string());
                                }
                            };
                            ...
                            // Build the derivation using n3v3-derive
                            ...
                            let special_fields =
                                ["name", "version", "system", "builder", "args", "outputs"];
                            ...
                            let out_path = drv
                                .out_path()
                                .map(|p| p.to_string())
                                .unwrap_or_else(|| format!("/n3v3/store/{}-{}", drv.hash(), name));

                            // Return the derivation as a record with computed fields
                            let mut result = (**attrs).clone();
                            result.insert(
                                "type".to_string(),
                                Value::String(Rc::new("derivation".to_string())),
                            );
                            result.insert(
                                "drvPath".to_string(),
                                Value::String(Rc::new(drv_path.to_string())),
                            );
                            result.insert(
                                "outPath".to_string(),
                                Value::String(Rc::new(out_path.clone())),
                            );
        ...
        (
            ...
            Value::Builtin(BuiltinFn {
                name: "toJSON",
                arity: 1,
                func: |args| Ok(Value::String(Rc::new(value_to_json(&args[0])))),
            }),
        ),
        (
            "fromJSON",
...
mod tests {
    ...
    fn typeof_reports_path_runtime_object() {
        let result = call_builtin(
            typeof_builtin(),
            &[Value::Path(Rc::new(PathBuf::from("/tmp/n3v3")))],
        );
    ...
    fn typeof_reports_redirect_runtime_object() {
        let result = call_builtin(
            typeof_builtin(),
            &[Value::redirect_stdout_path_object("/tmp/n3v3.out")],
        );
    ...
    fn format_value_keeps_redirect_opaque() {
        let rendered = super::format_value(&Value::redirect_stdout_path_object("/tmp/n3v3.out"));
