...
//! 标准库源码获取辅助函数。
//!
//! These helpers expose `n3v3-fetch` to n3v3 programs so package expressions
//! can materialize sources with optional hash verification.
//! 这些辅助函数把 `n3v3-fetch` 暴露给 n3v3 程序，使包表达式可按需获取源码并可选校验哈希。

use n3v3_derive::Hash;
use n3v3_eval::value::{BuiltinFn, Value};
use n3v3_fetch::{FetchResult, Fetcher, Source};
...
fn default_fetch_cache_dir() -> PathBuf {
    if let Ok(dir) = std::env::var("N3V3_FETCH_CACHE") {
        return PathBuf::from(dir);
    }

    if let Ok(xdg_cache) = std::env::var("XDG_CACHE_HOME") {
        return PathBuf::from(xdg_cache).join("n3v3").join("fetch");
    }
