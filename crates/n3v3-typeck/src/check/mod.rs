...
//! 类型检查器实现。
//!
//! This module implements the main type checker for n3v3.
...
use n3v3_diagnostic::{Diagnostic, DiagnosticKind, ErrorCode, Label, Severity};
use n3v3_hir::{
...
impl TypeChecker {
    ...
    /// 收集函数泛型参数的特征约束以供后续检查。
    fn collect_fn_bounds(&mut self, def_id: DefId, fn_def: &n3v3_hir::FnDef) {
    ...
    fn visit_effect_references(
        ...
        match &expr.kind {
            ...
            ExprKind::Interpolated(parts) => {
                for part in parts {
                    if let n3v3_hir::StringPart::Expr(value) = part {
                        self.visit_effect_references(value, include_method_fallbacks, visit);
                    }
                }
            }
