...
mod tests {
    use super::*;
    use neve_hir::TraitItem;

    fn make_int_ty() -> Ty {
        Ty { kind: TyKind::Int, span: Span::DUMMY }
    }

    fn make_string_ty() -> Ty {
    ...
    fn test_register_trait() {
        ...
        let trait_id = resolver.register_trait(DefId(0), &trait_def);
        
        assert!(resolver.lookup_trait("Show").is_some());
        assert_eq!(resolver.lookup_trait("Show"), Some(trait_id));
        
        let info = resolver.get_trait(trait_id).unwrap();
    ...
    fn test_register_impl() {
        ...
        let _trait_id = resolver.register_trait(DefId(0), &trait_def);
        
        // Register inherent impl for Int
        ...
        let impl_id = resolver.register_impl(DefId(1), &impl_def);
        
        let impls = resolver.find_inherent_impls(&make_int_ty());
        assert!(impls.contains(&impl_id));
    }
    ...
    fn test_find_trait_impl() {
        ...
        let trait_def = TraitDef {
            ...
            items: vec![
                TraitItem {
                    name: "eq".to_string(),
                    generics: vec![],
                    params: vec![make_int_ty()],
                    return_ty: Ty { kind: TyKind::Bool, span: Span::DUMMY },
                    default: None,
                    span: Span::DUMMY,
                },
            ],
        ...
        resolver.register_impl(DefId(1), &impl_def);
        
        // Should find impl
        let found = resolver.find_trait_impl(trait_id, &make_int_ty());
        assert!(found.is_some());
        
        // Should not find impl for String
        let not_found = resolver.find_trait_impl(trait_id, &make_string_ty());
    ...
    fn test_impl_completeness() {
        ...
        // Register trait with required method
        let trait_def = TraitDef {
            ...
            items: vec![
                ...
                TraitItem {
                    ...
                    params: vec![],
                    return_ty: make_string_ty(),
                    default: Some(neve_hir::Expr {
                        kind: neve_hir::ExprKind::Literal(neve_hir::Literal::String("debug".to_string())),
                        ty: make_string_ty(),
                        span: Span::DUMMY,
                    }), // Has default
                    span: Span::DUMMY,
        ...
        resolver.register_trait(DefId(0), &trait_def);
        
        // Incomplete impl - missing 'display'
        let trait_ty = Ty {
            kind: TyKind::Named(DefId(0), vec![]),
            span: Span::DUMMY,
        };
        let impl_def = ImplDef {
            ...
            trait_ref: Some(trait_ty),
            self_ty: make_int_ty(),
            items: vec![], // No methods!
            assoc_type_impls: vec![],
        };
        
        let impl_id = resolver.register_impl(DefId(1), &impl_def);
        
        let missing = resolver.check_impl_completeness(impl_id);
        assert!(missing.contains(&"display".to_string()));
        assert!(!missing.contains(&"debug".to_string())); // Has default
    }
    ...
    fn test_associated_types() {
        ...
        // Register trait with associated type (like Iterator)
        let trait_def = TraitDef {
            ...
            items: vec![
                TraitItem {
                    ...
                    generics: vec![],
                    params: vec![],
                    // Return type would reference Self::Item but simplified here
                    return_ty: Ty { kind: TyKind::Unknown, span: Span::DUMMY },
                    default: None,
                    span: Span::DUMMY,
                },
            ],
            assoc_types: vec![
                neve_hir::AssocTypeDef {
                    name: "Item".to_string(),
                    bounds: vec![],
                    default: None,
                    span: Span::DUMMY,
        ...
        // Check trait has associated type
        let assocs = resolver.trait_assoc_types(trait_id);
        assert_eq!(assocs.len(), 1);
        assert_eq!(assocs[0].name, "Item");
        
        // Register impl with associated type
        let trait_ty = Ty {
            kind: TyKind::Named(DefId(0), vec![]),
            span: Span::DUMMY,
        };
        let impl_def = ImplDef {
            generics: vec![],
            trait_ref: Some(trait_ty),
            self_ty: make_int_ty(), // Int implements Iterator (contrived)
            items: vec![],
            assoc_type_impls: vec![
                neve_hir::AssocTypeImpl {
                    name: "Item".to_string(),
                    ty: make_int_ty(), // Item = Int
                    span: Span::DUMMY,
                },
            ],
        };
        ...
        let resolved = resolver.resolve_assoc_type(&make_int_ty(), trait_id, "Item");
        assert!(resolved.is_some());
        match resolved.unwrap().kind {
            TyKind::Int => {}
    ...
    #[test]
    fn test_assoc_type_completeness() {
        let mut resolver = TraitResolver::new();
        
        // Register trait with required associated type
        let trait_def = TraitDef {
            ...
            assoc_types: vec![
                ...
                neve_hir::AssocTypeDef {
                    ...
                    bounds: vec![],
                    default: Some(make_int_ty()), // Has default
                    span: Span::DUMMY,
                },
            ],
        };
        ...
        // Incomplete impl - missing 'Element'
        let trait_ty = Ty {
            kind: TyKind::Named(DefId(0), vec![]),
            span: Span::DUMMY,
        };
        let impl_def = ImplDef {
            ...
            self_ty: make_int_ty(),
            items: vec![],
            assoc_type_impls: vec![], // Missing Element!
        };
        
        let impl_id = resolver.register_impl(DefId(1), &impl_def);
        
        let missing = resolver.check_impl_assoc_types(impl_id);
        assert!(missing.contains(&"Element".to_string()));
        assert!(!missing.contains(&"Index".to_string())); // Has default
        
        // Full completeness check
        let completeness = resolver.check_impl_full_completeness(impl_id);
        assert!(!completeness.is_complete());
        assert!(completeness.missing_assoc_types.contains(&"Element".to_string()));
    }
}
