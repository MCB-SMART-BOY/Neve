...
use crate::builtin_types::{
    ...
    builtin_result, builtin_set, builtin_task, is_builtin_option_type, is_builtin_result_type,
};
use crate::errors::{
...
impl TypeChecker {
    ...
    fn suggest_global_name(&self, _span: Span) -> String {
        ...
        if all_known.len() <= max_suggestions {
            ...
            let preview: Vec<&str> = all_known.iter().take(max_suggestions).map(|s| s.as_str()).collect();
            format!(
    ...
    fn global_name_for_did(&self, def_id: DefId) -> Option<String> {
        self.global_names.get(&def_id).cloned()
    }
