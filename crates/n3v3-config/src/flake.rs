//! Flake support for n3v3.
//! n3v3 的 Flake 支持。
//!
//! Flakes provide a standard way to define reproducible n3v3 projects
//! with explicit dependencies and outputs.
//!
//! Flake 提供了一种标准方式来定义具有明确依赖和输出的可复现 n3v3 项目。
//!
//! A flake is defined by a `flake.n3v3` file in the project root that exports:
//! Flake 由项目根目录中的 `flake.n3v3` 文件定义，该文件导出：
//!
...
use n3v3_diagnostic::{Diagnostic, Severity};
...
use n3v3_frontend::{FrontendDriver, ProgramAnalysis, analyze_source};
...
pub enum FlakeOutput {
    ...
    DevShell(Value),
    /// A NixOS/n3v3 system configuration. / NixOS/n3v3 系统配置。
    System(Value),
    /// A home-manager configuration. / home-manager 配置。
    HomeConfig(Value),
    /// An overlay. / 覆盖层。
    Overlay(Value),
    /// An n3v3 module. / n3v3 模块。
    Module(Value),
    /// A template. / 模板。
    Template(Value),
...
impl Flake {
    ...
    pub fn load(root: &Path) -> Result<Self, ConfigError> {
        let flake_file = root.join("flake.n3v3");
        if !flake_file.exists() {
            return Err(ConfigError::Flake(format!(
                "no flake.n3v3 found in {}",
                root.display()
            )));
        }
    ...
    fn try_resolve_input_flake_value(
        ...
        if !source_root.join("flake.n3v3").exists() {
            return Ok(None);
        }
    ...
    fn materialize_git_source(
        ...
        if !checkout_dir.exists() {
            let work_dir = temp_work_dir("n3v3-flake-input-git")?;
        ...
        Ok(if checkout_dir.join("flake.n3v3").exists() {
    ...
    fn materialize_archive_source(
        ...
        if !extract_dir.exists() {
            ...
            let work_dir = temp_work_dir("n3v3-flake-input-archive")?;
        ...
        Ok(if root.join("flake.n3v3").exists() {
...
fn resolve_source_module_path(path: &Path) -> Result<(PathBuf, Vec<String>), ConfigError> {
    ...
    let mut segments: Vec<String> = rel_path
        ...
        .map(|component| {
            ...
            if part.ends_with(".n3v3") {
                part.trim_end_matches(".n3v3").to_string()
            } else {
                part
            }
        })
...
pub fn init_flake(root: &Path, description: Option<&str>) -> Result<Flake, ConfigError> {
    ...
    outputs = fn(inputs) {{
        packages = {{
            x86_64_linux = {{
                default = inputs.n3v3.packages.x86_64_linux.hello,
            }},
        }}
    }}
}};
"#,
        description.unwrap_or("An n3v3 flake")
    );

    std::fs::write(root.join("flake.n3v3"), flake_content)?;
...
fn resolve_git_source(url: &str, git_ref: &str) -> Result<(String, Hash), ConfigError> {
    let work_dir = temp_work_dir("n3v3-flake-git")?;
    let clone_path = work_dir.join("repo");
...
fn detected_archive_root(root: &Path) -> PathBuf {
    if root.join("flake.n3v3").exists() {
...
fn flake_source_cache_dir() -> Result<PathBuf, ConfigError> {
    let path = std::env::temp_dir().join("n3v3-flake-source-cache");
...
mod tests {
    ...
    fn test_init_flake_writes_canonical_source() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let source = fs::read_to_string(temp.path().join("flake.n3v3"))?;
