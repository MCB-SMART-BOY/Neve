...
use n3v3_diagnostic::{Diagnostic, DiagnosticKind};
use n3v3_syntax::{self, SourceFile, Visibility};
...
pub(crate) fn collect_imports(
    ...
    file.items
        ...
        .filter_map(|item| match &item.kind {
            n3v3_syntax::ItemKind::Import(import_def) => {
                let prefix = match import_def.prefix {
                    n3v3_syntax::PathPrefix::Absolute => crate::ImportPathPrefix::Absolute,
                    n3v3_syntax::PathPrefix::Self_ => crate::ImportPathPrefix::Self_,
                    n3v3_syntax::PathPrefix::Super => crate::ImportPathPrefix::Super,
                    n3v3_syntax::PathPrefix::Crate => crate::ImportPathPrefix::Crate,
                    _ => return None,
                };
                ...
                let kind = match &import_def.items {
                    n3v3_syntax::ImportItems::Module => ImportKind::Module,
                    n3v3_syntax::ImportItems::Items(items) => {
                        ImportKind::Items(items.iter().map(|i| i.name.clone()).collect())
                    }
                    n3v3_syntax::ImportItems::All => ImportKind::All,
                    _ => {
