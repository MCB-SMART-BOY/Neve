...
use n3v3_syntax::{self as ast, SourceFile};
...
impl Resolver {
    ...
    /// 在降级过程中使用，类型将在稍后推断。
    #[inline]
    fn unknown_ty(span: n3v3_common::Span) -> Ty {
        Ty {
            kind: TyKind::Unknown,
            span,
        }
    ...
    fn lower_name_expr(&self, name: &str, span: n3v3_common::Span) -> Expr {
        Expr {
    ...
    fn lower_top_level_let(&mut self, item: &ast::Item, def: &ast::LetDef) -> Vec<Item> {
        ...
        let mut items = vec![Item {
            ...
            kind: ItemKind::Fn(FnDef {
                name: format!("__n3v3_pattern_value_{}", source_id.0),
                generics: Vec::new(),
                params: Vec::new(),
                return_ty: source_ty,
                effectful: false,
