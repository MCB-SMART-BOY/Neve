...
pub use n3v3_syntax::Visibility;
...
impl ModuleLoader {
    ...
    /// 获取已加载模块的解析源文件。
    pub fn parsed_source(&self, module_id: ModuleId) -> Option<&n3v3_syntax::SourceFile> {
    ...
    pub fn load_module(&mut self, path: &[String]) -> Result<ModuleId, ModuleLoadError> {
        ...
        for item in &source_file.items {
            if let n3v3_syntax::ItemKind::Import(import_def) = &item.kind {
                let Some(import_path) = ModulePath::from_import_def(import_def) else {
                    self.diagnostics.push(Diagnostic::error(
                        n3v3_diagnostic::DiagnosticKind::Module,
                        item.span,
    ...
    pub fn discover_modules(&mut self) -> Result<Vec<ModuleId>, ModuleLoadError> {
        ...
        // Start with lib.n3v3 or main.n3v3
        // 从 lib.n3v3 或 main.n3v3 开始
        let root_dir = self.root_dir().to_path_buf();
        let _root_file = if root_dir.join("lib.n3v3").exists() {
            root_dir.join("lib.n3v3")
        } else if root_dir.join("main.n3v3").exists() {
            root_dir.join("main.n3v3")
        } else if root_dir.join("src/lib.n3v3").exists() {
            root_dir.join("src/lib.n3v3")
        } else if root_dir.join("src/main.n3v3").exists() {
            root_dir.join("src/main.n3v3")
        } else {
    ...
    fn discover_submodules(
        ...
        for entry in entries {
            ...
            if path.is_file() && file_name.ends_with(".n3v3") && file_name != "mod.n3v3" {
                let module_name = file_name.trim_end_matches(".n3v3");
                ...
                // Check for mod.n3v3 in subdirectory
                // 检查子目录中的 mod.n3v3
                let mod_file = path.join("mod.n3v3");
                if mod_file.exists() {
                    let mut module_path = parent_path.to_vec();
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            ModuleLoadError::NoRootModule => {
                write!(f, "no root module found (lib.n3v3 or main.n3v3)")
            }
