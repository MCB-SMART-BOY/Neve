...
impl IncrementalCache {
    ...
    pub(crate) fn parse_source(
        ...
        let (file, diagnostics) = n3v3_parser::parse(source);
...
mod tests {
    ...
    fn temp_file_path(name: &str) -> PathBuf {
        let unique = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        env::temp_dir().join(format!("n3v3_hir_incremental_{unique}_{name}"))
    }

    #[test]
    fn cache_stats_track_recompile_checks() {
        let file_path = temp_file_path("main.n3v3");
    ...
    fn parse_source_cache_retains_parsed_ast_and_diagnostics() {
        let file_path = temp_file_path("parse.n3v3");
        let mut cache = IncrementalCache::default();
