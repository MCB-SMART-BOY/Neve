...
mod tests {
    ...
    fn graph_tracks_reverse_dependents() {
        let mut graph = ModuleGraphState::default();
        graph.register_module(info(0, &["a"], "/tmp/a.n3v3", None));
        graph.register_module(info(1, &["b"], "/tmp/b.n3v3", None));
        graph.register_module(info(2, &["c"], "/tmp/c.n3v3", None));
