...
impl ModulePath {
    ...
    pub fn from_import_def(import: &n3v3_syntax::ImportDef) -> Option<Self> {
        let segments: Vec<String> = import.path.iter().map(|i| i.name.clone()).collect();
        match import.prefix {
            n3v3_syntax::PathPrefix::Absolute => Some(Self::absolute(segments)),
            n3v3_syntax::PathPrefix::Self_ => Some(Self::self_(segments)),
            n3v3_syntax::PathPrefix::Super => Some(Self::super_(segments)),
            n3v3_syntax::PathPrefix::Crate => Some(Self::crate_(segments)),
            _ => None,
        }
    }
...
impl ModulePathResolver {
    ...
    /// Append the n3v3 extension without replacing dots in a module stem.
    /// 在不替换模块文件名中点号的情况下追加 n3v3 扩展名。
    fn append_n3v3_extension(path: PathBuf) -> PathBuf {
        let Some(file_name) = path.file_name() else {
            return path.with_extension("n3v3");
        };
    ...
    pub(crate) fn find_module_file(&self, module_path: &[String]) -> Option<PathBuf> {
        if module_path.is_empty() {
            return Some(self.root_dir.join("lib.n3v3"));
        }
            ...
            && let Some(std_path) = &self.std_path
        {
            ...
            let file_path = Self::append_n3v3_extension(std_path.join(&relative));
            if file_path.exists() {
                return Some(file_path);
            }

            let mod_path = std_path.join(&relative).join("mod.n3v3");
            if mod_path.exists() {
        ...
        let file_path = Self::append_n3v3_extension(self.root_dir.join(&relative));
        if file_path.exists() {
            return Some(file_path);
        }

        let mod_path = self.root_dir.join(&relative).join("mod.n3v3");
        if mod_path.exists() {
            return Some(mod_path);
        }

        let src_path = Self::append_n3v3_extension(self.root_dir.join("src").join(&relative));
        if src_path.exists() {
        ...
        {
            if rest.is_empty() {
                // Import of the flake input root itself: look for lib.n3v3 or mod.n3v3
                let lib_path = input_root.join("lib.n3v3");
                if lib_path.exists() {
                    return Some(lib_path);
                }
                let mod_path = input_root.join("mod.n3v3");
                if mod_path.exists() {
                    return Some(mod_path);
                }
                return None;
            ...
            let flake_file = Self::append_n3v3_extension(input_root.join(&flake_relative));
            if flake_file.exists() {
                return Some(flake_file);
            }
            let flake_mod = input_root.join(&flake_relative).join("mod.n3v3");
            if flake_mod.exists() {
                return Some(flake_mod);
            }
            let flake_src =
                Self::append_n3v3_extension(input_root.join("src").join(&flake_relative));
            if flake_src.exists() {
...
mod tests {
    ...
    #[test]
    fn test_find_module_file_preserves_dotted_module_stem() {
        let root = std::env::temp_dir().join(format!("n3v3_dotted_module_{}", std::process::id()));
        let nested_dir = root.join("a");
        std::fs::create_dir_all(&nested_dir).expect("create nested module directory");
        std::fs::write(root.join("a.b.n3v3"), "// dotted module").expect("write dotted module");
        std::fs::write(nested_dir.join("b.n3v3"), "// nested module").expect("write nested module");

        let resolver = ModulePathResolver::new(&root);
        assert_eq!(
            resolver.find_module_file(&["a.b".into()]),
            Some(root.join("a.b.n3v3"))
        );
        assert_eq!(
            resolver.find_module_file(&["a".into(), "b".into()]),
            Some(nested_dir.join("b.n3v3"))
        );
    ...
    fn test_find_module_file_in_flake_input() {
        let tmp = std::env::temp_dir().join(format!("n3v3_flake_test_{}", std::process::id()));
        let input_root = tmp.join("dep");
        std::fs::create_dir_all(input_root.join("src")).expect("create src dir");
        std::fs::write(input_root.join("lib.n3v3"), "// dep lib").expect("write lib.n3v3");
        std::fs::write(input_root.join("utils.n3v3"), "// dep utils").expect("write utils.n3v3");
        std::fs::write(
            input_root.join("src").join("helpers.n3v3"),
            "// dep helpers",
        )
        .expect("write helpers.n3v3");
        ...
        // Should find lib.n3v3 via flake input root (empty rest → looks for lib.n3v3)
        let found = resolver.find_module_file(&["mydep".into()]);
        assert!(found.is_some(), "should find mydep via flake input");
        assert!(
            found.unwrap().ends_with("lib.n3v3"),
            "should resolve to lib.n3v3"
        );

        // Should find utils.n3v3 via flake input root
        let found = resolver.find_module_file(&["mydep".into(), "utils".into()]);
        assert!(found.is_some(), "should find mydep.utils via flake input");

        // Should find src/helpers.n3v3 via flake input root
    ...
    fn test_find_module_file_flake_nested_path() {
        let tmp = std::env::temp_dir().join(format!("n3v3_flake_nested_{}", std::process::id()));
        ...
        std::fs::write(
            input_root.join("deep").join("nested").join("target.n3v3"),
            "// nested",
        )
        .expect("write target.n3v3");
        std::fs::write(
            input_root
                .join("src")
                .join("deep")
