# n3v3-typeck: Type System

## Architecture

```
Resolved HIR Module (from n3v3-hir)
       │
       ▼
┌─────────────────────────────────────────────────────────┐
...
The checker emits `n3v3_diagnostic::Diagnostic` values with
`DiagnosticKind::Type`. Error-severity diagnostics block `n3v3 check`; warning
diagnostics do not. In particular, the duplicate-definition message
`duplicate definition of \`name\` shadows previous` is a type diagnostic
(`DiagnosticKind::Type`, `Severity::Warning`), as are unreachable-pattern and
callable-fallback warnings.
...
| From | To | Data |
|------|----|------|
| n3v3-hir | n3v3-typeck | `Module` (resolved, pre-type) |
| n3v3-typeck | n3v3-eval | Checked `Module` (ready for evaluation) |
| n3v3-typeck | n3v3-lsp | Type map + diagnostics for hover/completion |
| n3v3-typeck | n3v3-cli | `n3v3 check` diagnostics |

## Key Files

| File | What |
|------|------|
| `crates/n3v3-typeck/src/check/mod.rs` | Main checker, inference passes, effect propagation, and `TypeChecker` |
| `crates/n3v3-typeck/src/check/builtin_type.rs` | Built-in type checking |
| `crates/n3v3-typeck/src/infer.rs` | Hindley-Milner W-algorithm |
| `crates/n3v3-typeck/src/unify.rs` | Unification + substitution |
| `crates/n3v3-typeck/src/builtin_types.rs` | Built-in type signatures |
| `crates/n3v3-typeck/src/pattern_analysis.rs` | Pattern coverage and exhaustiveness |
| `crates/n3v3-typeck/src/traits.rs` | Trait resolution + method dispatch |
| `crates/n3v3-typeck/src/errors.rs` | Diagnostic builders for type errors |
